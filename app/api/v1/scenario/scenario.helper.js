...
const getWaitingOn = (practice) => {
  ...
  if (hasLearnerSentMessage && hasLearnerEvaluated && hasCoachEvaluated) {
    waitingOn = _NOONE
  }
...
module.exports.validateCreate = (data) => {
  ...
  if (data.type && data.type === 'current') {
    result = joi.validate(data, schemaCurrent)
    ...
    return {
      ...
      coachMark: practice.coachMark.length
        ? await Promise.all(practice.coachMark.map(async mark => {
          const criteria = await daoCriteria.findById(mark._criteria)
          const criteriaName = criteria ? criteria.name : null
          return {
            mark: mark.mark,
