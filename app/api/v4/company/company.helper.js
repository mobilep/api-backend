...
module.exports.validateInviteUsers = (data) => {
  const schema = joi.array().items(joi.object().keys({
    ...
    postcode: joi.string().trim().min(1).max(30).allow('').required(),
...
module.exports.validateSendTestEmail = (data) => {
  const schema = joi.object().keys({
    ...
    lang: joi.string().lowercase().valid('en', 'fr', 'ru', 'it', 'es', 'de', 'ja', 'ko', 'ch', 'pt').required()
  })
