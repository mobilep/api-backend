...
describe('COMPANY API v4', () => {
  describe('POST /api/v4/company', () => {
    let sysAdmin
    mocha.before((done) => {
    ...
    it('should create company on /api/v4/company POST', (done) => {
      const data = {
        name: 'Test1',
        info: 'it company',
        sendReminder: 3
      ...
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          res.body.should.have.property('sendReminder')
          res.body.name.should.be.equal(data.name)
          res.body.info.should.be.equal(data.info)
          res.body.sendReminder.should.be.equal(data.sendReminder)
          done()
        })
    })

    it('should create company on /api/v4/company POST (diacritic)', (done) => {
      const data = {
        name: 'āā ȯ& 0-9 _ & - .',
        info: 'it company',
        sendReminder: 3
    ...
    it('should not create company on /api/v4/company POST (diacritic)',
      (done) => {
        const data = {
          name: 'āā ȯ(',
          info: 'it company'
  ...
  describe('POST /api/v4/company/:companyId/user', () => {
    ...
    it('should invite user(s) to company if company admin', (done) => {
      ...
      chai.request(server)
        .post('/api/v4/company/' + company1 + '/user')
    ...
    it('should invite user(s) to company if sys admin', (done) => {
      ...
      chai.request(server)
        .post('/api/v4/company/' + company2 + '/user')
        .set('Authorization', sysAdmin.accessToken)
        .send(data)
        .end((err, res) => {
          chai.should().not.exist(err)
  ...
  describe('GET /api/v4/company/:companyId', () => {
    const company1 = '57fe2450916165b0b8b20aaa'
    const company2 = '57fe2450916165b0b8b20bbb'
    let companyAdmin, sysAdmin
    const notAdmin = {
  ...
  describe('POST /api/v4/company/:companyId/invite', () => {
    const company1 = '57fe2450916165b0b8b20aaa'
    let sysAdmin
    mocha.before((done) => {
      chai.request(server).put('/api/v4/auth').send({
    ...
    it('should send invite email to user(s) if they exists', (done) => {
      ...
      chai.request(server)
        .post('/api/v4/company/' + company1 + '/invite')
        .set('Authorization', sysAdmin.accessToken)
        .send(data)
        .end((err, res) => {
          chai.should().not.exist(err)
  ...
  describe('PATCH /api/v4/company/:companyId', () => {
  ...
  describe('DELETE /api/v4/company/:companyId', () => {
