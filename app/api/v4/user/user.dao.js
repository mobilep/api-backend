...
module.exports.create = (data) => {
  return userModel.findOne({email: data.email.toLowerCase()}, {lean: true}).then((existingEmail) => {
    ...
    if (data._coach) {
      return this.getByEmailAndCompany(data._coach, data._company)
    }
    return true
  }).then((existingCoach) => {
    if (!existingCoach) {
...
module.exports.getByIdWithDetails = (id) => {
  return userModel.findOne({_id: id, isActive: true}).populate('_company').catch((err) => {
    throw utils.ErrorHelper.serverError(err)
  })
}
...
module.exports.edit = (id, data) => {
  ...
  return userModel.findOne({_id: {$ne: id}, email: email})
    ...
    .then((user) => {
      ...
      user.postcode = (data.postcode || data.postcode === '') ? data.postcode : user.postcode
